                ...
                <div class="input-field col s6" id="divInt" {if $edit.linkType != "lnkInt"}style="display: none;"{/if}>
                    <select id="selInt" title="Intern" name="lnkIntern">
                        <optgroup label="Seiten">
                            {loop $sites}
                                <option {if $id == $_.edit.lnkVal}selected{/if} value="p{$id}">{$title}</option>
                            {/loop}
                        </optgroup>
                        <optgroup label="Protokolle">
                            {loop $protocols}
                                <option {if $id == $_.edit.lnkVal}selected{/if} value="pr{$id}">{$title}</option>
                            {/loop}
                        </optgroup>
                        <optgroup label="Dateien">
                            {loop $files}
                                <option {if $id == $_.edit.lnkVal}selected{/if} value="f{$id}">{$title}</option>
                            {/loop}
                        </optgroup>
                    </select>
