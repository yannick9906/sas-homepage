...
<core-scaffold>
    ...
    <?php
        ...
        $res = $link->query("SELECT * FROM AKs");
        while($row = mysqli_fetch_object($res)) {
            echo <<<END
    <paper-shadow z="1" class="card">
        <a href="AKDetail.php?id=$row->ID"><core-icon style="height: 64px; width: 64px; color: black;"icon="$row->Icon"></core-icon></a>
        <a href="AKDetail.php?id=$row->ID"><h2>$row->Name</h2></a>
        <p>$row->textshort</p>
    </paper-shadow>
END;
